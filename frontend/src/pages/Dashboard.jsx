...
export default function Dashboard({ mode = 'aoo' }) {
  ...
  return <div className="min-h-screen bg-[#f6f8fc] text-slate-900">
    ...
    <div className="lg:pl-72">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1920px] items-center justify-between px-4 py-4 sm:px-7"><Link to={isDirecteur ? '/directeur' : '/dashboard'} className="flex items-center gap-3 lg:hidden"><span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-blue-700 to-indigo-700 text-sm font-black text-white">DR</span><span><b className="block text-sm">DRCA · RSK</b><small className="text-xs text-slate-500">{isDirecteur ? 'Espace de direction' : 'Appels d’offres'}</small></span></Link><div className="hidden lg:block"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Plateforme sécurisée</p><p className="mt-1 flex items-center gap-2 text-sm font-bold text-slate-700"><ShieldCheck size={16} className="text-emerald-600" /> {isDirecteur ? 'Espace décisionnel Directeur' : 'Cycle des appels d’offres'}</p></div><div className="flex items-center gap-2.5"><button type="button" onClick={() => { if (currentUser?.role === 'directeur') { navigate('/directeur'); } else { openLoginModal({ title: 'Espace Directeur', subtitle: 'Authentification du Directeur requise.', requiredRole: 'directeur', onSuccess: () => navigate('/directeur') }); } }} className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white text-xs font-bold rounded-xl shadow-xs transition transform active:scale-98 cursor-pointer" title="Accéder à l'Espace Directeur"><ShieldCheck size={15} /><span>Directeur</span></button><button type="button" onClick={() => navigate('/login', { state: { redirect: '/bons-commande', title: 'Bons de commande' } })} className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition transform active:scale-98 cursor-pointer" title="Accéder aux Bons de commande"><Receipt size={15} /><span>Bons de commande</span></button><button onClick={() => setMobileNavOpen(!mobileNavOpen)} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600 lg:hidden"><Menu size={18} /></button></div></div></header>

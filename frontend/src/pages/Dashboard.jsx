...
export default function Dashboard({ mode = 'aoo' }) {
  ...
  const moduleNavigation = [
    { label: 'Appels d’offres', icon: FileText, route: '/aoos' },
  ...
  return <div className="min-h-screen bg-[#f6f8fc] text-slate-900">
    ...
    <div className="lg:pl-72">
      ...
      {mobileNavOpen && <div className="fixed inset-x-4 top-16 z-50 rounded-2xl border border-slate-200 bg-white p-3 shadow-xl lg:hidden">{isDirecteur ? <div className="space-y-1">{navigation.map(({ label, icon: Icon, action }) => <button key={label} onClick={() => { action(); setMobileNavOpen(false); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700"><Icon size={18} />{label}</button>)}</div> : <div className="space-y-1">{moduleNavigation.map(({ label, icon: Icon, route }) => <Link key={route} to={route} onClick={() => setMobileNavOpen(false)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700"><Icon size={18} />{label}</Link>)}</div>}<button onClick={logout} className="mt-2 flex w-full items-center gap-3 rounded-xl border-t border-slate-100 px-3 py-3 text-left text-sm font-semibold text-rose-600"><LogOut size={18} /> Se déconnecter</button></div>}
      <main className="mx-auto max-w-[1920px] px-4 py-7 sm:px-7"><div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="text-sm font-semibold text-blue-700">DIRECTION RÉGIONALE</p><h1 className="mt-1 text-3xl font-bold tracking-tight">{isDirecteur ? 'Tableau de bord Directeur' : 'Tableau de bord des appels d’offres'}</h1><p className="mt-2 text-sm text-slate-500">{isDirecteur ? 'Vue consolidée des activités, procédures et indicateurs budgétaires.' : 'Vue consolidée du cycle : consultation, engagement, liquidation et ordonnancement.'}</p></div><div className="flex flex-wrap gap-2"><label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 hover:border-slate-300 transition-colors"><CalendarDays size={16} className="text-blue-600" /><select value={year} onChange={(event) => setYear(event.target.value)} className="bg-transparent font-semibold outline-none cursor-pointer">{availableYears.map((y) => <option key={y} value={y}>{y}</option>)}</select></label><select value={month} onChange={(event) => setMonth(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 outline-none"><option value="">Tous les mois</option>{months.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select><button className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600"><SlidersHorizontal size={16} /> Filtres</button></div></div>

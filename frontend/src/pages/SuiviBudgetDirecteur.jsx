...
export default function SuiviBudgetDirecteur() {
  ...
  return <main className="min-h-screen bg-slate-50 p-4 text-slate-900 sm:p-7"><div className="mx-auto max-w-[1800px]">
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <PhaseCard step="1" label="Crédits notifiés" value={summary?.total_notifie} rateValue="Taux : 100 %" detail="Base de calcul" Icon={BarChart3} tone="bg-blue-600" />
      <PhaseCard step="2" label="Engagement" value={summary?.total_engage} rateValue={`Taux / notifié : ${rate(summary?.taux_consommation || pct(summary?.total_engage, summary?.total_notifie))}`} detail="Engagé / notifié" Icon={FileCheck2} tone="bg-emerald-600" />
      <PhaseCard step="3" label="Ordonnancement" value={summary?.total_ordonnance} rateValue={`Taux / notifié : ${rate(summary?.taux_ordonnancement_notifie || pct(summary?.total_ordonnance, summary?.total_notifie))}`} secondaryRate={`Taux / engagé : ${rate(summary?.taux_ordonnancement || pct(summary?.total_ordonnance, summary?.total_engage))}`} detail="Ordonnancé / notifié & engagé" Icon={FileText} tone="bg-violet-600" />

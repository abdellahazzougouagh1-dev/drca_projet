...
import { ShoppingBag, ArrowLeft, Clock, Sparkles, FileText, Landmark } from 'lucide-react';

export default function GestionBonDeCommande() {
  return (
      ...
      <main className="min-w-0 flex-1 px-5 py-10 sm:px-10 lg:px-14 lg:py-14 flex items-center justify-center">
        <div className="max-w-2xl w-full text-center space-y-8">
          ...
          <div className="bg-white rounded-3xl p-10 border border-slate-200 shadow-xl space-y-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-40 h-40 bg-blue-600/5 rounded-full blur-3xl pointer-events-none"></div>

            <div className="grid h-20 w-20 place-items-center rounded-3xl bg-blue-100 text-blue-700 mx-auto shadow-inner">
              <ShoppingBag size={42} />
            </div>
            ...
            <div className="space-y-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-blue-100 text-blue-800 border border-blue-200">
                <Clock size={14} className="text-blue-600" /> Module en développement
              </span>
              <h1 className="text-3xl font-extrabold text-[#1e3a8a]">Bon de Commande</h1>
              <p className="text-base text-slate-600 leading-relaxed max-w-lg mx-auto">
                Le module <strong>Bon de Commande</strong> est en cours de développement. Cette fonctionnalité sera disponible prochainement dans une version ultérieure.
              </p>
            </div>

            <div className="pt-4 border-t border-slate-100 flex items-center justify-center gap-4 text-xs font-semibold text-slate-500">
              <span className="flex items-center gap-1.5"><Sparkles size={14} className="text-blue-600" /> Architecture prête</span>
              <span>•</span>
              <span>Workflow indépendant à venir</span>
            </div>
          </div>
        </div>
      </main>
